"""
Tests for Deal Payments Feature (Task 86af6ykhh) — service integration + data.

Covers: plan_fact_service integration from alive code paths, data-structure
invariants for paid/unpaid plan-fact items, variance calculation, and edge
cases around the alive _finance_plan_fact_tab_content helper.

Note: Route-level tests (TestPaymentRoutesExist, TestHelperFunctionsExist,
TestDealDetailShowsPaymentsSection, TestPaymentFormRoute,
TestRegisterPaymentRoute, TestPaymentPostValidation,
TestNoUnpaidItemsEdgeCase, TestDeletePaymentRoute, TestPaymentRouteAuth,
TestPaymentRegistrationFormData, TestPaymentHTMXIntegration) were removed
during Phase 6C-2B-10c1 archive of /finance + payment routes (2026-04-20).
Their target handlers (GET/POST /finance/{deal_id}/payments/new, POST
/finance/{deal_id}/payments, DELETE /finance/{deal_id}/payments/{item_id},
_payment_registration_form) now live in legacy-fasthtml/finance_lifecycle.py
