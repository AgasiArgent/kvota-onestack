...
import {
  ...
  deleteQuoteExpense,
  fetchQuoteExpenses,
  type CustomsQuoteExpense,
} from "@/entities/customs-expense";

interface QuoteCustomsExpensesProps {
  quoteId: string;
  /** Current roles — "Добавить" hidden when user can't mutate. */
  userRoles: string[];
}
...
/**
 * Per-quote customs overhead: broker fees, ДТ filing, СВХ, etc.
 * RUB-only. Visible to all org members; mutations gated by customs role.
 */
export function QuoteCustomsExpenses({
  quoteId,
  userRoles,
}: QuoteCustomsExpensesProps) {
  const [expenses, setExpenses] = useState<CustomsQuoteExpense[]>([]);
  ...
  useEffect(() => {
    let cancelled = false;
    fetchQuoteExpenses(quoteId).then((rows) => {
      if (!cancelled) {
        setExpenses(rows);
        setLoaded(true);
      }
  ...
  async function handleAdd() {
    ...
    startTransition(async () => {
      try {
        const { expense_id } = await createQuoteExpense({
          quote_id: quoteId,
          label,
          amount_rub: amount,
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between gap-2 space-y-0 pb-3">
        <CardTitle className="text-sm">Расходы по таможне (вся КП)</CardTitle>
      ...
      <CardContent className="flex flex-col gap-2">
        ...
        {loaded && expenses.length === 0 && (
          <div className="py-1 text-xs text-muted-foreground">
            Расходов пока нет. Добавьте услуги брокера, ДТ, СВХ и прочие затраты.
          </div>
        )}
        ...
        {canWrite && (
          <div className="mt-1 flex flex-wrap items-center gap-2 border-t border-border pt-3">
            <Input
              placeholder="Наименование (например, брокер)"
              value={newLabel}
