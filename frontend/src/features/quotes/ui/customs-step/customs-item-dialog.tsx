          ...
          {showCertSection && (
            ...
            >
              ...
              {attachedCertViews.length > 0 ? (
                ...
                /* Empty state — amber card with TWO buttons per REQ-8 AC#3:
                   «Привязать к существующему» (popover) +
                   «Создать новый» (`<Button variant="default">`). The
                   spec copy is «Сертификат соответствия не оформлен»
                   (mockup line 904). The Phase B Wave 5 hotfix added the
                   missing «Создать новый» button — pre-hotfix the
                   empty-state shipped with only the bind trigger, leaving
                   half of the AC#3 entry path unreachable. */
                <div
                    ...
                    <div className="flex flex-wrap gap-2">
                      <CertificateBindPopover
                        ...
                        onCreateNew={() => {
                          ...
                          // Empty-state popover-internal «Создать новый»
                          // opens the create-cert modal with no preset; the
                          // current item lands pre-selected via
                          // `preSelectedItemIds`.
                          setCertModalPreset(null);
                          setCreateCertModalOpen(true);
                        }}
                      ...
                      <Button
                        ...
                        onClick={() => {
                          // Phase B Wave 5 hotfix — REQ-8 AC#3.
                          // Direct «Создать новый» entry from the empty
                          // state. Same wiring as the popover-internal
                          // path (no preset; current item pre-selected).
                          setCertModalPreset(null);
                          setCreateCertModalOpen(true);
                        }}
                      >
