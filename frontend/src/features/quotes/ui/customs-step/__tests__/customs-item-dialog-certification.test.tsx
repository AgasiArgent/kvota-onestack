...
// ---------------------------------------------------------------------------
// Phase B Wave 5 hotfix — Bug 3: empty-state must offer TWO buttons
// ---------------------------------------------------------------------------
//
// REQ-8 AC#3 mandates the «Сертификация» empty-state expose both
// «Привязать к существующему» (popover trigger) AND «Создать новый»
// (`<Button variant="default">`) actions. Pre-hotfix only the bind
// trigger shipped — half of the AC entry path was unreachable from the
// per-item dialog.
//
// String-scan tests because the workspace has no jsdom; the empty-state
// branch is a static slice of JSX. We assert the source file contains the
// two button labels, the testid for the new button, and the wiring to the
// existing `setCreateCertModalOpen`/`setCertModalPreset` state hooks (so a
// future refactor that breaks the wiring fails CI without needing a real
// DOM).
// ---------------------------------------------------------------------------

describe("CustomsItemDialog — empty-state Bug 3 regression (REQ-8 AC#3)", () => {
  let dialogSrc = "";
  ...
  beforeEach(async () => {
    ...
    dialogSrc = await fs.readFile(dialogPath, "utf-8");
  });

  it("empty-state copy matches the spec: «Сертификат соответствия не оформлен»", () => {
    // REQ-8 AC#3 — exact copy from mockup line 904. Pre-hotfix the file
    // shipped «Эта позиция ещё не привязана…» which drifts from spec.
    expect(dialogSrc).toContain("Сертификат соответствия не оформлен");
    // The drifted string must be gone.
    expect(dialogSrc).not.toContain(
      "Эта позиция ещё не привязана ни к одному сертификату",
    );
  });

  it("renders both «Привязать к существующему» and «Создать новый» buttons", () => {
    // REQ-8 AC#3 mandates two actions in the empty-state. The spec wording
    // is «Привязать к существующему» (no trailing «сертификату» — that was
    // the pre-hotfix copy drift on the trigger label).
    expect(dialogSrc).toContain("Привязать к существующему");
    expect(dialogSrc).toContain("Создать новый");
    // The new dedicated button must have its testid for browser-tests.
    expect(dialogSrc).toContain(
      'data-testid="customs-item-dialog-cert-create-new"',
    );
    // The pre-hotfix combined trigger label is gone.
    expect(dialogSrc).not.toContain(
      "Привязать к существующему сертификату",
    );
  });

  it("«Создать новый» click opens CertificateModal with the same wiring as the popover variant", () => {
    // Both entry paths (popover-internal onCreateNew and the new direct
    // button) must clear the preset and open the modal. The modal mount
    // already wires `preSelectedItemIds={[currentItemForSelect.id]}` —
    // the pre-existing assertion in the orphan-removal block above
    // covers that. Here we anchor on the new button's onClick using the
    // testid as a search anchor.
    const buttonIdx = dialogSrc.indexOf(
      'data-testid="customs-item-dialog-cert-create-new"',
    );
    expect(buttonIdx).toBeGreaterThan(0);
    // Slice forward ~400 chars from the testid — the onClick handler
    // sits in the same JSX block. Both setters must be referenced.
    const slice = dialogSrc.slice(buttonIdx, buttonIdx + 600);
    expect(slice).toContain("setCertModalPreset(null)");
    expect(slice).toContain("setCreateCertModalOpen(true)");
  });

  it("uses Button variant=\"default\" for «Создать новый» (REQ-8 AC#3)", () => {
    // Spec calls for `<Button variant="default">` to make the create-new
    // action visually primary. The bind trigger uses `variant="outline"`
    // — that pre-existing styling stays unchanged.
    const buttonIdx = dialogSrc.indexOf(
      'data-testid="customs-item-dialog-cert-create-new"',
    );
    expect(buttonIdx).toBeGreaterThan(0);
    // Look BACKWARDS to the opening `<Button` tag.
    const before = dialogSrc.slice(Math.max(0, buttonIdx - 200), buttonIdx);
    expect(before).toMatch(/variant=["']default["']/);
  });
});
