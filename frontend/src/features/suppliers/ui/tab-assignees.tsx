...
import type { SupplierAssignee } from "@/entities/supplier/types";
import {
  addSupplierAssignee,
  removeSupplierAssignee,
} from "@/entities/supplier/mutations";

interface Props {
  supplierId: string;
  assignees: SupplierAssignee[];
  procurementUsers: { id: string; full_name: string }[];
  canManage: boolean;
}

export function TabAssignees({ supplierId, assignees, procurementUsers, canManage }: Props) {
  ...
  const availableUsers = procurementUsers.filter((u) => !assignedUserIds.has(u.id));

  async function handleAdd() {
    if (!selectedUserId) return;
    setAdding(true);
    try {
      await addSupplierAssignee(supplierId, selectedUserId);
      setSelectedUserId("");
      router.refresh();
    } catch (err) {
  ...
  async function handleRemove(userId: string) {
    ...
    try {
      await removeSupplierAssignee(supplierId, userId);
      router.refresh();
    } catch (err) {
      ...
      {assignees.length === 0 ? (
        <div className="py-12 text-center">
          ...
          <p className="text-xs text-text-subtle">
            {canManage
              ? "Добавьте менеджеров отдела закупок, ответственных за этого поставщика"
