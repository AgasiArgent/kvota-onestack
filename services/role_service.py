...
- roles: Reference table with role definitions (9 predefined roles)
- user_roles: Junction table linking users to roles per organization (supports multi-role)
"""
...
def get_user_roles(user_id: str | UUID, organization_id: str | UUID) -> List[Role]:
    ...
    This uses the user_roles junction table which supports multiple roles per user.
...
def assign_role(
    ...
    This uses the user_roles table which supports multiple roles per user.
...
def remove_role(
    ...
    This deletes the entry from user_roles table.
...
def get_role_by_code(code: str) -> Optional[Role]:
    """
    Get a single role by its code (slug in DB).
...
def get_users_by_role(organization_id: str | UUID, role_code: str) -> List[dict]:
    ...
    # Query user_roles for this role in the organization
    response = supabase.table("user_roles") \
...
def get_users_by_any_role(organization_id: str | UUID, role_codes: List[str]) -> List[dict]:
    ...
    # Get role IDs for the specified codes (DB uses 'slug')
    roles_response = supabase.table("roles") \
        .select("id, slug") \
        .in_("slug", role_codes) \
        .execute()

    role_id_map = {item["id"]: item["slug"] for item in roles_response.data}
