...
4. WHEN the request sets `impl_status` AND the caller does NOT hold one of roles [`admin`, `head_of_sales`, `head_of_procurement`, `head_of_logistics`, `sales`, `procurement`, `procurement_senior`, `logistics`, `customs`], THEN the API SHALL return HTTP 403 with body `{ "success": false, "error": { "code": "FORBIDDEN_FIELD", "message": "Role <slug> cannot write field impl_status" } }`. Line-level executor roles (sales, procurement, procurement_senior, logistics, customs) are included because they build the features and are best-placed to mark their own implementation as done / partial / missing; qa_status remains reserved for the QA tier (Req 6.5).
