-- Migration: Create buyer_companies table
-- Feature: DB-002
...
ON buyer_companies(organization_id, company_code);

-- Indexes for common queries
CREATE INDEX IF NOT EXISTS idx_buyer_companies_organization
ON buyer_companies(organization_id);

CREATE INDEX IF NOT EXISTS idx_buyer_companies_name
ON buyer_companies(organization_id, name);

CREATE INDEX IF NOT EXISTS idx_buyer_companies_active
ON buyer_companies(organization_id, is_active) WHERE is_active = true;

CREATE INDEX IF NOT EXISTS idx_buyer_companies_inn
ON buyer_companies(organization_id, inn);
...
CREATE POLICY buyer_companies_select_policy ON buyer_companies
    FOR SELECT
    USING (
        organization_id IN (
...
CREATE POLICY buyer_companies_insert_policy ON buyer_companies
    FOR INSERT
    WITH CHECK (
        organization_id IN (
...
CREATE POLICY buyer_companies_update_policy ON buyer_companies
    FOR UPDATE
    USING (
        organization_id IN (
            SELECT ur.organization_id
...
CREATE POLICY buyer_companies_delete_policy ON buyer_companies
    FOR DELETE
    USING (
        organization_id IN (
            SELECT ur.organization_id
...
CREATE OR REPLACE FUNCTION update_buyer_companies_timestamp()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
...
DROP TRIGGER IF EXISTS buyer_companies_update_timestamp ON buyer_companies;
CREATE TRIGGER buyer_companies_update_timestamp
    BEFORE UPDATE ON buyer_companies
    FOR EACH ROW
    EXECUTE FUNCTION update_buyer_companies_timestamp();

-- ============================================
-- COMMENTS
-- ============================================

COMMENT ON TABLE buyer_companies IS 'Our legal entities used for purchasing from suppliers (v3.0)';
COMMENT ON COLUMN buyer_companies.company_code IS '3-letter unique code for the buyer company within organization';
COMMENT ON COLUMN buyer_companies.inn IS 'Russian tax ID (ИНН) - 10 digits for legal entities';
COMMENT ON COLUMN buyer_companies.kpp IS 'Russian tax registration code (КПП) - 9 digits';
COMMENT ON COLUMN buyer_companies.ogrn IS 'Russian state registration number (ОГРН) - 13 digits';
COMMENT ON COLUMN buyer_companies.registration_address IS 'Legal registration address of the company';
COMMENT ON COLUMN buyer_companies.general_director_name IS 'Name of the general director for document signing';
COMMENT ON COLUMN buyer_companies.general_director_position IS 'Position title of the director (default: Генеральный директор)';
