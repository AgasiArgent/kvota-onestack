-- Migration 023: Create bank_accounts table
-- Part of DB-006 feature: Polymorphic bank accounts for all entity types
...
-- Comments
COMMENT ON TABLE bank_accounts IS 'Банковские реквизиты для всех типов компаний (полиморфная таблица)';
COMMENT ON COLUMN bank_accounts.entity_type IS 'Тип сущности: supplier, buyer_company, seller_company, customer';
COMMENT ON COLUMN bank_accounts.entity_id IS 'ID сущности в соответствующей таблице';
COMMENT ON COLUMN bank_accounts.bank_name IS 'Наименование банка';
COMMENT ON COLUMN bank_accounts.account_number IS 'Расчётный счёт';
COMMENT ON COLUMN bank_accounts.bik IS 'БИК банка (9 цифр, только для РФ)';
COMMENT ON COLUMN bank_accounts.correspondent_account IS 'Корреспондентский счёт (20 цифр)';
COMMENT ON COLUMN bank_accounts.swift IS 'SWIFT/BIC код для международных переводов';
COMMENT ON COLUMN bank_accounts.iban IS 'IBAN для международных переводов';
COMMENT ON COLUMN bank_accounts.currency IS 'Валюта счёта: RUB, USD, EUR, TRY, CNY, etc.';
COMMENT ON COLUMN bank_accounts.is_default IS 'Основной счёт по умолчанию для данной сущности';
COMMENT ON COLUMN bank_accounts.is_active IS 'Активный/архивный статус';
...
CREATE INDEX idx_bank_accounts_entity
    ON bank_accounts(entity_type, entity_id);
...
CREATE INDEX idx_bank_accounts_default
    ON bank_accounts(entity_type, entity_id, is_default)
    WHERE is_default = true AND is_active = true;

-- Search by currency
CREATE INDEX idx_bank_accounts_currency
    ON bank_accounts(organization_id, currency)
...
COMMENT ON FUNCTION get_default_bank_account IS 'Получить основной банковский счёт для сущности';

-- Get all bank accounts for an entity
CREATE OR REPLACE FUNCTION get_entity_bank_accounts(
    p_entity_type VARCHAR,
    p_entity_id UUID
)
RETURNS TABLE (
...
COMMENT ON FUNCTION get_entity_bank_accounts IS 'Получить все банковские счета для сущности';
