-- Migration 127: Create ERPS Registry SQL View
-- Purpose: Consolidated view for "Единый реестр подписанных спецификаций" with all calculated fields

-- Drop view if exists (for safe re-run)
DROP VIEW IF EXISTS kvota.erps_registry;

-- Create ERPS Registry View
...
-- Grant SELECT to authenticated users (RLS policies will handle org filtering)
GRANT SELECT ON kvota.erps_registry TO authenticated;

-- Add comment
COMMENT ON VIEW kvota.erps_registry IS 'ERPS Registry - Единый реестр подписанных спецификаций with all auto-calculated fields for finance dashboard';
