-- Migration 161: Fix ERPS Registry View spec_sum_usd showing $0.00
-- Purpose: Use q.total_amount_usd instead of the old broken column from quote_calculation_summaries
-- Bug: The old column from quote_calculation_summaries is often NULL,
--       causing spec_sum_usd to show $0.00. q.total_amount_usd (from quotes table)
--       is populated by the calculation engine save logic and is the correct source.
...
COMMENT ON VIEW kvota.erps_registry IS 'ERPS Registry - Единый реестр подписанных спецификаций with all fields including delivery dates and comments. Fixed: uses q.total_amount_usd for spec totals';
